# CData Dremio ARP Connector for FTP Servers

The FTP Servers connector allows Dremio to connect to and query data in FTP Servers. This can then allow you to build custom reports, dashboards, or even just ad-hoc SQL via your client tool of choice. Note that it does require a third-party JDBC driver that is not free, but does allow a free trial.
...
3. Download and install the [FTP Servers JDBC Driver from CData](https://www.cdata.com/drivers/ftp/download/jdbc)* and copy the JAR file to the /jars/3rdparty/ directory of your Dremio installation. For example:
